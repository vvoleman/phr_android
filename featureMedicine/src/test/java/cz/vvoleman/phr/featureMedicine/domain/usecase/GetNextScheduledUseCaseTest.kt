...
class GetNextScheduledUseCaseTest {
    ...
    fun `Get next schedule when multiple schedules at the same time`() = runTest {
        ...
        assertEquals("3", actualValue[0].medicine.id, "Next scheduled medicine should be medicine with id 3")
        assertEquals("2", actualValue[1].medicine.id, "Next scheduled medicine should be medicine with id 2")
