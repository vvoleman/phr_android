...
interface MedicineDao {
    ...
    fun getAll(): Flow<List<MedicineWithDetails>>

    @Query("SELECT * FROM medicine WHERE id IN (:medicineIds)")
    fun getManyById(medicineIds: List<String>): Flow<List<MedicineWithDetails>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(medicine: MedicineDataSourceModel)
